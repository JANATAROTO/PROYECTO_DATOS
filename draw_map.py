...
def draw_polygon():
    longitudes = []
    latitudes = []
    area = pd.read_csv('C:\proyecto_2estructuras\ST0245\codigo\entrega2\src\poligono_de_medellin.csv', sep=';')
...
def draw_map(path):
    ...
    gmp_draw.polygon(limit_latitude, limit_longitude, face_color='white', face_alpha=0.4, edge_color='grey',
                     edge_width=5)
    gmp_draw.scatter(lat, long, 'black', size=3, marker=False)
    gmp_draw.plot(lat, long, 'black', edge_width=4)
